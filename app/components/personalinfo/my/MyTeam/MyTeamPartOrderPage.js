...
export default class MyTeamPartOrderPage extends BaseComponent{
    ...
    render(): React.ReactNode {
     ...
     header(){
        return (
            <View>
                <HeaderComponent name={'1'}/>
                <View style={{flexDirection: 'row',backgroundColor:'#FFF',marginTop:5,marginBottom:1,height:36,alignItems:'center'}}>
                    <View style={{width:2,height:15,marginLeft:10,marginRight:8,backgroundColor:'#fa3314'}}></View>
                    <Text style={{fontSize:15,color:'#303030'}}>下单员列表</Text>
                </View>
     ...
     Refresh(){
         fetch( HTTP_REQUEST.Host+'/promoter/promoter/orderManList.do', {
             method: 'POST',
             headers: {
    ...
    _renderItem = ({item}) => (
        <View style={teamStyle.itemlayout}>
            ...
            <View style={{flex:1,marginRight:120,}}>
                ...
                <Text>合计会员数: {item.numberOfMembers}</Text>
            ...
            <TouchableOpacity style={teamStyle.orderList} onPress={()=>this.props.navigation.navigate("MyTeamMemberPage",{orderManId:item.orderManId})}>
                <Text style={{color:'#FF1D1D'}}>会员列表</Text>
            </TouchableOpacity>
        </View>
    );
