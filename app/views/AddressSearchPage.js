...
/**
 * 地址搜索选择页，用户搜索所在小区名称。根据结果选择。
 */
export default class AddressSearchPage extends Component{
    ...
    renderListItem = ({item}) => (
        <TouchableOpacity
            activeOpacity={0.7}
            onPress={()=>{
                this.props.navigation.state.params.onAddressSelect(item.name,item.fullName,item.smallCommunityId);
    ...
    searchAddress(){
        fetch(HTTP_REQUEST.Host+'/area/area/getSmallCommunityByKey.do', {
            method: 'POST',
            headers: {
