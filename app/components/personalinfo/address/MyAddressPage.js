...
export default class MyAddressPage extends Component{
    ...
    createAddress(){
        if(this.state.addressList.length >= 2){
            this.refs.toast.show("您只能拥有两个收货地址，不能新增!",1000);
            return;
        }
        this.props.navigation.navigate(
            'CreateAddressPage',
            {refresh:()=>this._getDefaultAddress()}
        )
    }
    ...
    render(){
        return(
            <View style={{backgroundColor:'#F5F5F5',flex:1}}>
                ...
                <TouchableOpacity
                    activeOpacity={0.7}
                    style={{backgroundColor:"#EC7E2D",height:55,justifyContent:'center',alignItems:'center'}}
        ...
        return (
            <View key={index} style={{backgroundColor:'#FFF',marginBottom:8}}>
                <View style={addressStyle.itemTop}>
                    <Text style={addressStyle.tv1}>
                        {item.receiverName+"      "}( {item.sex === 'FEMALE'?'女士':'先生'} ){"      "+item.mobile}
                    </Text>
                    <View style={{height:5}}/>
                ...
                <View style={addressStyle.itemBottom}>
                    <TouchableOpacity
                        onPress={()=>this.changeDefault(item,index)}
                        style={{color:'#000',flexDirection:'row'}}>
                        <Image
                    ...
                    <TouchableOpacity
                        ...
                        <Image
                            style={{width:15,height:16,marginRight:5}}
                            source={{uri:'http://qnm.laykj.cn/image/shanchu.png'}}/>
       ...
       .then((responseJson)=>{
           if(responseJson.respCode === 'S'){
               ...
               for(let i=0;i<responseJson.data.length;i++){
                   ...
                   bean.select = bean.receiveAddressId === this.state.defaultAddressId;
                   tempList.push(bean)
               }
